...
num_columns = st.sidebar.number_input('Number of condition(s)', min_value=1, max_value=None, value=1, step=1, help=None, key="mix")
...
if vector_for_all_mix:
    vector_per_mix = st.sidebar.number_input('Vector per condition', min_value=1, max_value=len(df), value=1, step=1,help=None,
    ...
    if selected_vector:
        for j in range(1, vector_per_mix + 1):
            ...
            if amount_dna_for_all:
                ...
                st.sidebar.warning("Amount of vector available only if you select the option to define the amount of DNA desired")
...
if culture_vessel_for_all:
    culture_vessel = st.sidebar.selectbox("Culture Vessel", list(culture_vessel_options_lipo.keys()), key=f"culture_vessel")
...
try:
    for i in range(1, num_columns + 1):
        with columns[i % num_columns - 1]:
            ...
            culture_vessel = st.selectbox("Culture Vessel",
                                          list(culture_vessel_options_lipo.keys()) if culture_vessel_for_all is False else [culture_vessel],
            ...
            culture_vessel_value = culture_vessel_options_lipo[culture_vessel] if transfection_type == "Lipofectamine (2000/3000)"\
                else culture_vessel_options_jetprime[culture_vessel]
            for j in range(1, vector_per_mix + 1):
                vector_selected = st.selectbox(f'Vector {j}',
                                               df["Plasmid/Vector/RNA"] if selected_vector is False else [st.session_state[f"vector_selected{j}"]],
                ...
                if max_slider > 0:
                    amount_dna_selected = st.slider(
                        f'Amount of {vector_selected if vector_selected is not None or vector_selected != "" else f"Vector {j}"} (µg)',
                        0.0, max_slider, value=0.0 if selected_vector is False or amount_dna_for_all is False else st.session_state[f"amount_vector_selected{j}"],
    ...
    for i, amount_of_dna, transfection_type, ratio, culture_vessel_value, number_wells, vector_selected, amount_dna_selected in calcul:
        if vector_selected not in results_dict:
            ...
            results_dict[vector_selected][f"Condition {i} (µL)"] = (amount_dna_selected / next(item['µg/µL'] for item in vector_table
                                                                                         if item["Plasmid/Vector/RNA"] == vector_selected)) * number_wells
        ...
        if transfection_type == "Lipofectamine (2000/3000)":
            if "Culture Vessel Lipo 1" not in results_dict:
                results_dict["Culture Vessel Lipo 1"] = {
                    "Plasmid/Vector/RNA": "OptiMEM w/ Plasmid/Vector/RNA", f"Condition {i} (µL)": culture_vessel_value * number_wells}
                results_dict["Culture Vessel Lipo 2"] = {
                    "Plasmid/Vector/RNA": "OptiMEM w/ Lipofectamine", f"Condition {i} (µL)": culture_vessel_value * number_wells}
            else:
            ...
            if "Culture Vessel JP" not in results_dict:
                results_dict["Culture Vessel JP"] = {"Plasmid/Vector/RNA": "jetPRIME Buffer", f"Condition {i} (µL)": culture_vessel_value * number_wells}
            else:
